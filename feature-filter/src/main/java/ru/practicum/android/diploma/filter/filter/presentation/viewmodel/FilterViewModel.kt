...
class FilterViewModel(
    ...
    fun updateProfessionInDataFilter(industryModel: IndustryModel) {
        viewModelScope.launch(Dispatchers.IO) { filterSPInteractor.updateProfessionInDataFilter(industryModel).collect{ value ->
            if (value==-1) {handleError()}
        }} }

    fun updateDoNotShowWithoutSalaryInDataFilter(switch: Boolean) {
        viewModelScope.launch(Dispatchers.IO) { filterSPInteractor.updateDoNotShowWithoutSalaryInDataFilter(switch).collect{ value ->
            if (value==-1) {handleError()}
        }} }

    fun updateSalaryInDataFilter(salaryExpectation: Int) {
        viewModelScope.launch(Dispatchers.IO) { filterSPInteractor.updateSalaryInDataFilter(salaryExpectation).collect{ value ->
            if (value==-1) {handleError()}
        }} }
