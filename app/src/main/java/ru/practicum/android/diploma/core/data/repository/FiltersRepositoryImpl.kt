...
class FiltersRepositoryImpl(val context: Context) : FiltersRepository {
    ...
    override val filters: Flow<Filters> = _filters.asStateFlow()//примененные фильтры, для экрана поиска
    private val _tempFilters = MutableStateFlow(Filters())
    override val tempFilters: Flow<Filters> = _tempFilters.asStateFlow()//Настраиваемые фильтры, для экранов Настройки Фильтров, выбор индурстрии, выбор региона
    init {
        initFilters()
    }
