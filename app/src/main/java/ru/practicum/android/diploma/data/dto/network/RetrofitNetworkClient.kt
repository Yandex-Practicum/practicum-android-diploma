...
class RetrofitNetworkClient(
    ...
    private val retrofit = Retrofit.Builder().baseUrl(BASE_URL).client(headerClient).build()
            ...
            when (dto) {
                is VacancySearchRequest -> {
                    withContext(Dispatchers.IO) {
                        try {
                            hhApiService.getVacancies(dto.searchQuery).apply { resultCode = HTTP_OK_CODE }
