...
class FilterViewModel(
    ...
    private fun processResult(industries: List<Industries>?, error: Int?) {
        if (industries != null) {
            Log.d("HH_TEST", "Industries count: ${industries.size}")
            for (indus in industries) {
                Log.d("HH_TEST", "Industries: ${indus.id} ${indus.name}")
                for (detail in indus.industries) {
                    Log.d("HH_TEST", "    IndustriesDetail: ${detail.id} - ${detail.name}")
                }
            }
        }
        if (error != null) {
            Log.d("HH_TEST", "Error: $error")
        }
    }
}
